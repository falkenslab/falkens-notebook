...
Técnica de prompting para agentes que combina razonamiento (*reasoning*) y acción (*acting*) en un bucle iterativo. El modelo alterna entre pensar en voz alta (qué debe hacer y por qué) y ejecutar una acción (llamar a una herramienta, buscar información), usando el resultado de cada acción para guiar el siguiente paso de razonamiento.

- Propuesto en el paper "ReAct: Synergizing Reasoning and Acting in Language Models" (Yao et al., 2022).
- Patrón típico: `Pensamiento → Acción → Observación → Pensamiento → ...` hasta llegar a la respuesta final.
- Ventaja frente a Chain of Thought puro: el agente puede consultar información real durante el razonamiento, reduciendo alucinaciones.

Ver también: [Agente IA](#agente-ia), [Chain of Thought](#chain-of-thought-cot), [Tool calling](#tool-calling).
