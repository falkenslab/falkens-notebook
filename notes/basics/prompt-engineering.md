...
## Experimento: el impacto de añadir un rol y contexto

**Contexto:** ver cómo cambia la calidad de respuesta entre un prompt mínimo y uno bien construido.
...
```
Puedes añadir manejo de errores para el caso en que y sea 0, ya que la división
por cero lanzará una excepción. También podrías añadir type hints.
```

**Prompt con rol, contexto y restricciones:**

```
Actúa como revisor de código Python senior.
El proyecto usa Python 3.12 y sigue PEP 8 estrictamente.
Revisa esta función e indica exactamente qué falta, con el código corregido:

def calcular(x, y):
    return x / y

Devuelve: (1) qué falla, (2) código corregido, (3) por qué importa.
```

**Resultado:**

```
1. Qué falla: no hay validación de división por cero ni type hints.
...
3. Por qué importa: sin la guarda, una llamada calcular(5, 0) lanza ZeroDivisionError
   sin mensaje descriptivo. El ValueError con mensaje explícito facilita el debugging.
```

**Qué aprender:** el segundo prompt tiene exactamente el mismo problema pero extrae una respuesta tres veces más accionable. El rol orienta el tono, el contexto reduce ambigüedad, y el formato explícito garantiza una salida estructurada.
